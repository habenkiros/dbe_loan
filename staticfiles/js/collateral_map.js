...
(function (global) {
  ...
  function declaredIcon(far) {
    return L.divIcon({
      className: 'collateral-map-pin collateral-map-pin-declared' + (far ? ' far' : ''),
      html: '<span title="Declared address">🏠</span>',
      iconSize: [26, 26],
      iconAnchor: [13, 26],
    });
  }
  ...
  function drawMarkers(map, layerGroup, data) {
    ...
    (data.markers || []).forEach(function (m) {
      ...
      if (m.kind === 'site') {
        icon = siteIcon();
      } else if (m.kind === 'declared_address') {
        icon = declaredIcon(m.far_from_site);
      } else {
        icon = photoIcon(m.far_from_site);
      }
