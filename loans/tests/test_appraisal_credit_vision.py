...
class AppraisalCreditVisionTests(TestCase):
    def setUp(self):
        ...
        for order, (key, name) in enumerate(QUALITATIVE_FACTOR_KEYS):
            best = QUALITATIVE_RATING_CHOICES_BY_FACTOR[key][0]
    ...
    def test_vision_lock_excludes_rate_workbook(self):
        ...
        self.assertEqual(FEATURE_SCHEMA_VERSION, 'appraisal_features_v1')
    ...
    def test_scorecard_and_features(self):
        ...
        self.assertEqual(card['schema'], 'credit_scorecard_v1')
        self.assertEqual(len(card['pillars']), 4)
